    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pins is None:
            pins = []
        pins = [
            copy(p) for p in pins
        ]  # Make copy so user's IcPin instances don't change
        for pin in pins:
        ...
        for side in ["L", "R", "T", "B"]:
            ...
            slots = [
                int(p.split("/")[1]) for p in slotnames if p is not None
            ] + [0]
            ...
            for i, pin in enumerate(sidepins[side]):
                ...
                if pin.slot is not None:
                    ...
                    if tot == 1:
                        ...
                        z = (
                            linspace(
                                1 / (tot + 2), 1 - 1 / (tot + 2), num=tot
                            )[num - 1]
                            * sidelen
                        )

                pinxy = {
                    "L": Point((0, z + edgepadH)),
                    "R": Point((w, z + edgepadH)),
                    "T": Point((z + edgepadW, h)),
                    "B": Point((z + edgepadW, 0)),
                }.get(side, Point((0, 0)))
                ...
                if side == "T" and slant > 0:
                    pinxy = Point(
                        (
                            pinxy[0],
                            pinxy[1]
                            - pinxy[0] * math.tan(-math.radians(slant)),
                        )
                    )
                ...
                if pin.pin and pin.pin != "":
                    ...
                    pofst = {
                        "L": Point((-plblofst - invertradius * 2, plblofst)),
                ...
                if leadlen > 0:
                    if pin.invert:
                        ...
                        self.segments.append(
                            SegmentCircle(pinxy + invertofst, invertradius)
                        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(pins // 2):
            ...
            # Left
            self.segments.append(
                SegmentPoly(((0, y1), (0, y2), (-pinw, y2), (-pinw, y1)))
            )
            pnum = pins // 2 - i
            ...
            if number:
                self.segments.append(
                    SegmentText(
                        (-pinw / 2, ymid), str(pnum), fontsize=pfontsize
                    )
                )
            if names:
                self.segments.append(
            ...
            if number:
                self.segments.append(
                    SegmentText(
                        (width + pinw / 2, ymid), str(pnum), fontsize=pfontsize
                    )
                )
            if names:
                self.segments.append(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        pins = [
            ...
            IcPin(
                "$\overline{Q}$", side="right", slot="1/2", anchorname="Qbar"
            ),
        ]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        pins = [
            ...
            IcPin(
                "$\overline{Q}$", side="right", slot="1/3", anchorname="Qbar"
            ),
        ]
...
def sevensegdigit(
    ...
    fillD = (
        segcolor
        if str(digit).lower()
        in ["2", "3", "5", "6", "8", "9", "0", "b", "c", "d", "e"]
        else None
    )
    fillE = (
        segcolor
        if str(digit).lower()
        in ["2", "6", "8", "0", "a", "b", "c", "d", "e", "f"]
        else None
    )
    fillF = (
        segcolor
        if str(digit).lower()
        in ["4", "5", "6", "8", "9", "0", "a", "b", "c", "e", "f"]
        else None
    )
    fillG = (
        segcolor
    ...
    if labelsegments:
        segments.append(
            SegmentText(
                segAlabel, "a", align=("center", "bottom"), fontsize=10
            )
        )
        segments.append(
            SegmentText(
                segGlabel, "g", align=("center", "bottom"), fontsize=10
            )
        )
        segments.append(
            SegmentText(
                segDlabel, "d", align=("center", "bottom"), fontsize=10
            )
        )
        segments.append(
        ...
        **kwargs,
        ...
        pins = [
            IcPin(
                pin="a",
                side="left",
                slot=f"{7+decimal}/{slots}",
                anchorname="a",
            ),
            IcPin(
                pin="b",
                side="left",
                slot=f"{6+decimal}/{slots}",
                anchorname="b",
            ),
            IcPin(
                pin="c",
                side="left",
                slot=f"{5+decimal}/{slots}",
                anchorname="c",
            ),
            IcPin(
                pin="d",
                side="left",
                slot=f"{4+decimal}/{slots}",
                anchorname="d",
            ),
            IcPin(
                pin="e",
                side="left",
                slot=f"{3+decimal}/{slots}",
                anchorname="e",
            ),
            IcPin(
                pin="f",
                side="left",
                slot=f"{2+decimal}/{slots}",
                anchorname="f",
            ),
            IcPin(
                pin="g",
                side="left",
                slot=f"{1+decimal}/{slots}",
                anchorname="g",
            ),
        ]
        if decimal:
            pins.append(
                IcPin(
                    pin="dp", side="left", slot=f"1/{slots}", anchorname="dp"
                )
            )
        if anode:
