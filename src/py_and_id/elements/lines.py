...
class Arrow(Line):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            *d, arrowlength=headlength, arrowwidth=headwidth, **kwargs
        )
        self.double = double
...
class LineDot(Line):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(*d, **kwargs)
        warnings.warn(
            "LineDot is deprecated. Use Line().dot() or Line(arrow='-o')",
            DeprecationWarning,
        )
        zorder = kwargs.get("zorder", 4)
        self.params["fill"] = fill
        self.segments.append(
            SegmentCircle((0, 0), radius, ref="end", zorder=zorder)
        )
        if double:
            self.segments.append(
                SegmentCircle((0, 0), radius, ref="start", zorder=zorder)
            )
        # Explicitly define center so reverses work
        self.anchors["center"] = (0, 0)
...
class Dot(Element):
    ...
    def __init__(
        self, *d, radius: float = 0.075, open: bool = False, **kwargs
    ):
        super().__init__(*d, **kwargs)
...
class Arrowhead(Element):
    ...
    def __init__(
        self, *d, headwidth: float = 0.15, headlength: float = 0.25, **kwargs
    ):
        super().__init__(*d, **kwargs)
        self.segments.append(
            Segment(
...
class DotDotDot(Element):
    ...
    def __init__(
        ...
        fill = "bg" if open else True
        self.params["fill"] = fill
        self.segments.append(SegmentCircle((0.5, 0), radius))
...
class Wire(Element):
    ...
    def __init__(
        self, shape: str = "-", k: float = 1, arrow: str = None, **kwargs
    ):
...
class Annotate(Arc3):
    ...
    def _setlabel(self, dx, dy):
        ...
        if th2 < 45 or th2 > 315:
            ...
            self.params["lblofst"] = (0.1, 0)
        elif 45 <= th2 <= 135:
            self.params["lblalign"] = ("center", "bottom")
            self.params["lblofst"] = (0, 0.1)
        elif 135 < th2 <= 225:
...
class ArcZ(Arc3):
    ...
    def __init__(
        self, k=0.75, arrow=None, arrowlength=0.25, arrowwidth=0.2, **kwargs
    ):
        super().__init__(
            k=k,
            arrow=arrow,
        ...
        try:
            center = Point(
                (
                    (
                        xa
                        + math.sqrt(self.radius**2 - (q / 2) ** 2)
                        * (-dy / q)
                    ),
                    (
                        ya
                        + math.sqrt(self.radius**2 - (q / 2) ** 2) * (dx / q)
                    ),
                )
            )
...
class Tag(Element):
    ...
    def __init__(
        self, *d, width: float = 1.5, height: float = 0.625, **kwargs
    ):
        super().__init__(*d, **kwargs)
        height = height / 2
        self.segments.append(
...
class CurrentLabel(Element):
    ...
    def _place(self, dwgxy, dwgtheta, **dwgparams):
        ...
        self.segments.append(
            Segment((a, b), arrow="->", arrowwidth=0.2, arrowlength=0.3)
        )
        return super()._place(dwgxy, dwgtheta, **dwgparams)
...
class LoopCurrent(LoopArrow):
    ...
    def __init__(
        self,
        elm_list: Sequence[Element] = None,
        direction: Arcdirection = "cw",
        theta1: float = 35,
        theta2: float = -35,
        pad: float = 0.2,
...
class Rect(Element):
    ...
    def __init__(
        self, *d, corner1: XY = (0, 0), corner2: XY = (1, 1), **kwargs
    ):
        super().__init__(*d, **kwargs)
        c1a = (corner1[0], corner2[1])
        c2a = (corner2[0], corner1[1])
        self.segments.append(
            Segment([corner1, c1a, corner2, c2a, corner1], zorder=0)
        )
