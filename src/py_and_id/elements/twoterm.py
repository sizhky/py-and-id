...
class FlangeTrim(Flange):
    ...
    def __init__(self, *d, **kwargs):
        ...
        t1 = Point(
            (p2.x + tlen * math.cos(theta), p2.y + tlen * math.sin(theta))
        )
        t2 = Point(
            (
        ...
        self.segments.append(
            Segment(
                [
                    ...
                    (
                        resheight * 1.4 - schottky_width,
                        resheight - schottky_width,
                    ),
                ]
            )
        )
...
class Varactor(Element2Term):
    ...
    def __init__(self, *d, **kwargs):
        ...
        self.segments.append(
            SegmentPoly(
                [(0, resheight), (resheight * 1.4, 0), (0, -resheight)]
            )
        )
...
class PhotoCheckValve(CheckValve):
    ...
    def __init__(self, *d, **kwargs):
        ...
        self.segments.append(
            Segment(
                (p2[-2], pa2), arrow="->", arrowwidth=0.15, arrowlength=0.2
            )
        )
        self.params["lblloc"] = "bot"
...
class FuseUS(Element2Term):
    ...
    def fill(self, color: bool | str = True) -> "Element":
        ...
        fuser = 0.12
        self.segments.append(
            SegmentCircle((fuser, 0), fuser, zorder=4, fill=color)
        )
        self.segments.append(
...
class SparkGap(Element2Term):
    ...
    def __init__(self, *d, **kwargs):
        super().__init__(*d, **kwargs)
        self.segments.append(
            Segment([(0, 0), (0.3, 0), gap, (0.7, 0), (1, 0)])
        )
        # Arrow coords overlap a bit since default arrow is offset by linewidth
        self.segments.append(
            Segment([(0.3, 0), (0.52, 0)], arrow="->", arrowwidth=0.2)
        )
        self.segments.append(
            Segment([(0.7, 0), (0.48, 0)], arrow="->", arrowwidth=0.2)
        )
