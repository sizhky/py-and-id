...
class Element:
    ...
    def __init__(self, *d, **kwargs):
        ...
        self.params: MutableMapping[
            str, Any
        ] = {}  # Element-specific definition
        self._cparams: MutableMapping[
            str, Any
        ] = {}  # ChainMap of above params
        ...
        self.absanchors: MutableMapping[
            str, Any
        ] = {}  # Transformed, absolute anchors
    ...
    def theta(self, theta: float) -> "Element":
        ...
        if "d" in self._userparams:
            warnings.warn(
                f"Duplicate direciton parameter in element: `{self._userparams['d']}` replaced with `theta={theta}`"
            )
    ...
    def at(
        ...
        if "at" in self._userparams:
            warnings.warn(
                f"Duplicate `at` parameter in element: `{self._userparams['at']}` changed to `{xy}`."
            )
    ...
    def anchor(self, anchor: str) -> "Element":
        ...
        if "anchor" in self._userparams:
            warnings.warn(
                f"Duplicate anchor parameter in element: `{self._userparams['anchor']}` changed to `{anchor}`."
            )
    ...
    def _buildparams(self) -> None:
        ...
        if "at" in self._userparams and isinstance(
            self._userparams["at"][1], str
        ):
        ...
        self._cparams = ChainMap(
            self._userparams, self.params, self._dwgparams
        )
        self._flipreverse()
    ...
    def _place(
        self, dwgxy: XY, dwgtheta: float, **dwgparams
    ) -> tuple[Point, float]:
        ...
        lblsize = self._cparams.get(
            "lblsize", self._cparams.get("fontsize", 14)
        )
        ...
        for loc, label in kwlabels.items():
            if label is not None:
                ...
                self.label(
                    label, loc, fontsize=lblsize, rotate=rotate, color=lblcolor
                )
        ...
        if drop in self.anchors:
            ...
            if self.params.get("droptheta", None) is not None:
                ...
                self.absdrop = self.transform.transform(drop), self.params.get(
                    "droptheta"
                )
    ...
    def add_label(
        ...
        """
        warnings.warn(
            "`add_label` is deprecated. Use `label` instead.",
            DeprecationWarning,
        )
        if align is None:
            align = (None, None)
    ...
    def _place_label(
        ...
        if None in align:  # Determine best alignment for label based on angle
            ...
            th = (
                th + 360
            ) % 360  # Normalize angle so it's positive, clockwise
            ...
            if loc and loc in self.anchors:
                ...
                if (
                    math.isclose(self.anchors[loc][0], x1, abs_tol=0.15)
                    or math.isclose(self.anchors[loc][0], x2, abs_tol=0.15)
                    or math.isclose(self.anchors[loc][1], y1, abs_tol=0.15)
                    or math.isclose(self.anchors[loc][1], y2, abs_tol=0.15)
                ):
                    # Anchor is on an edge
                    dofst = self._cparams.get("lblofst", 0.1)

                    alignH: Halign
                    alignV: Valign
                    if math.isclose(self.anchors[loc][0], x1, abs_tol=0.15):
                        alignH = "right"
                        ofstx = -dofst
                    elif math.isclose(self.anchors[loc][0], x2, abs_tol=0.15):
                        alignH = "left"
                        ofstx = dofst
                    else:
                        alignH = "center"
                        ofstx = 0
                    if math.isclose(self.anchors[loc][1], y1, abs_tol=0.15):
                        alignV = "top"
                        ofsty = -dofst
                    elif math.isclose(self.anchors[loc][1], y2, abs_tol=0.15):
                        alignV = "bottom"
                        ofsty = dofst
                    else:
                        alignV = "center"
                    ...
                    try:
                        rotalignidx = (
                            rotalign.index(align) + round((th / 360) * 8)
                        ) % 8
                    except ValueError:
                        rotalignidx = 0
        ...
        if ofst is None:
            ofst = self._cparams.get("lblofst", 0.1)
        ...
        if isinstance(label, (list, tuple)):
            ...
            if loc == "top":
                ...
                ofst = (
                    Point((0, ofst))
                    if not isinstance(ofst, (list, tuple))
                    else Point(ofst)
                )
                for i, lbltxt in enumerate(label):
                    xy = Point((xmin + xdiv * (i + 1), ymax))
                    self.segments.append(
                        SegmentText(xy + ofst, lbltxt, **lblparams)
                    )
            elif loc == "bottom":
                xdiv = (xmax - xmin) / (len(label) + 1)
                ofst = (
...
class Element2Term(Element):
    ...
    def _place(
        ...
        if self._cparams.get("dot", False):
            fill: Union[bool, str] = (
                "bg" if self._cparams["dot"] == "open" else True
            )
            self.segments.append(
                SegmentCircle(end, radius=0.075, fill=fill, zorder=3)
            )
        if self._cparams.get("idot", False):
            fill = "bg" if self._cparams["idot"] == "open" else True
            self.segments.append(
                SegmentCircle(start, radius=0.075, fill=fill, zorder=3)
            )
