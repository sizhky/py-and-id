...
class RightLines(Element):
    ...
    def __init__(
        self, *d, n: int = 1, dy: float = 0.6, arrow: str = None, **kwargs
    ):
        super().__init__(*d, **kwargs)
        self._userparams["n"] = n
        self._userparams["dy"] = dy
    ...
    def _place(
        ...
        for i in range(n):
            ...
            self.segments.append(
                Segment([(0, y), (x, y), (x, dy)], arrow=arrow)
            )
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                if style == "square":
                    ...
                    self.segments.append(
                        SegmentCircle(
                            xy, pinrad * 1.75, fill=pinfill, zorder=4
                        )
                    )
                    self.segments.append(
                        Segment(
                    ...
                    self.segments.append(
                        SegmentCircle(xy, pinrad, fill=pinfill, zorder=4)
                    )
...
class Jumper(Element):
    ...
    def __init__(self, *d, pinspacing: float = 0.6, **kwargs):
        ...
        self.segments.append(
            SegmentPoly(
                [(-x, -x), (pinspacing + x, -x), (pinspacing + x, x), (-x, x)]
            )
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(n):
            y = -i * dy
            self.segments.append(
                Segment([(0, y), (dx - slantx, y), (dx, y + slanty)])
            )
            self.anchors["pin{}".format(i + 1)] = (0, y)
        self.segments.append(
            Segment([(dx, slantx), (dx, slanty - n * dy)], lw=lwbus)
        )
...
class DB9(Element):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.segments.append(
            SegmentPoly(
                [(0, 0), (0, h1), (w, h2), (w, -0.5)], cornerradius=0.25
            )
        )

        for i in range(4):
            xy = (edge, h1 - (i + 0.5) * pinspacing - edge)
            self.segments.append(
                SegmentCircle(xy, pinrad, fill=pinfill, zorder=4)
            )
        ...
        for i in range(5):
            xy = (edge + pinspacing, h2 - (i + 0.75) * pinspacing - edge)
            self.segments.append(
                SegmentCircle(xy, pinrad, fill=pinfill, zorder=4)
            )
...
class CoaxConnect(Element):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.segments.append(
            SegmentCircle((0, 0), radiusinner, fill=fillinner, zorder=4)
        )
