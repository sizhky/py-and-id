...
class ElementCompound(elm.Element):
    ...
    def move_from(
        self, xy: Point, dx: float = 0, dy: float = 0, theta: float = None
    ) -> None:
    ...
    def add(self, element: elm.Element, **kwargs) -> elm.Element:
        ...
        if not isinstance(element, elm.Element):
            ...
            warnings.warn(
                "kwargs to add method are ignored because element is already instantiated"
            )
        ...
        self.segments.extend(
            [
                s.xform(element.transform, **element._cparams)
...
class Optocoupler(ElementCompound):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if base:
            self.add(elm.Line(d="l", at=Point((B.get_bbox(transform=True).xmin, D.center[1])), l=0.15))  # type: ignore
    ...
    def __init__(
        ...
        **kwargs
        ...
        if core:
            self.add(
                elm.Line(
                    d="d", at=(Lleft + 0.15, -unit / 2), anchor="center", l=1
                )
            )
            swleft += 0.1
        ...
        if switch == "spst":
            ...
            bbox = SW.get_bbox()
            SW._userparams["at"] = (
                swleft - (-bbox.ymax if swflip else bbox.ymin),
                0,
            )
            self.add(SW)
            ...
            a = self.add(
                elm.Line(d="d" if swreverse else "u", at=SW.a, toy=toy)
            )
            toy = 0 if swreverse else -unit
            b = self.add(
                elm.Line(d="u" if swreverse else "d", at=SW.b, toy=toy)
            )
            c = self.add(
                elm.Line(d="u" if swreverse else "d", at=SW.c, toy=toy)
            )
            ...
            SW = elm.SwitchDpst(
                d="d", link=False, reverse=swreverse, flip=swflip
            )
            bbox = SW.get_bbox()
            SW._userparams["at"] = (
                swleft - (-bbox.ymax if swflip else bbox.ymin),
            ...
            toy = 0 if swreverse else -unit
            t1 = self.add(
                elm.Line(d="u" if swreverse else "d", at=SW.t1, toy=toy)
            )
            t2 = self.add(
                elm.Line(d="u" if swreverse else "d", at=SW.t2, toy=toy)
            )
            toy = -unit if swreverse else 0
            p1 = self.add(
                elm.Line(d="d" if swreverse else "u", at=SW.p1, toy=toy)
            )
            p2 = self.add(
                elm.Line(d="d" if swreverse else "u", at=SW.p2, toy=toy)
            )
            ...
            t3 = self.add(
                elm.Line(d="u" if swreverse else "d", at=SW.t3, toy=toy)
            )
            t4 = self.add(
                elm.Line(d="u" if swreverse else "d", at=SW.t4, toy=toy)
            )
            toy = -unit if swreverse else 0
            p1 = self.add(
                elm.Line(d="d" if swreverse else "u", at=SW.p1, toy=toy)
        ...
        if link:
            ...
            self.add(
                elm.Line(d="r", at=(Lleft + 0.2, -unit / 2), ls=":", tox=tox)
            )
        ...
        if box:
            ...
            self.add(
                elm.Rect(
                    d="r",
                    at=(0, 0),
                    corner1=(bbox.xmin - boxpad, bbox.ymin + 0.2),
                    corner2=(bbox.xmax + boxpad, bbox.ymax - 0.2),
                    fill=boxfill,
                    zorder=0,
                )
            )
...
class Wheatstone(ElementCompound):
    ...
    def __init__(
        self, vout: bool = False, labels: Sequence[str] = None, **kwargs
    ):
