...
class SwitchSpdt2(Element):
    ...
    def __init__(self, *d, action: ActionType = None, **kwargs):
        ...
        self.segments.append(
            Segment(
                [(0, 0), gap, (sw_dot_r * 2, 0.1), (0.7, 0.25), gap, (1, 0.4)]
            )
        )
...
class SwitchDpst(Element):
    ...
    def __init__(self, *d, link: bool = True, **kwargs):
        ...
        if link:
            self.segments.append(
                Segment([(0.5, yofst + 0.25), (0.5, 0.2)], ls=":")
            )
...
class SwitchDpdt(Element):
    ...
    def __init__(self, *d, link: bool = True, **kwargs):
        ...
        self.segments.append(
            SegmentCircle(
                (1 - sw_dot_r, yofst - 0.4), sw_dot_r, fill="bg", zorder=3
            )
        )
        self.segments.append(
            SegmentCircle(
        ...
        for i in range(n):
            ...
            self.segments.append(
                SegmentPoly(
                    (
                        (x, spacing + swidth),
                        (x + swidth, spacing + swidth),
                        (x + swidth, spacing + swidth * 2),
                        (x, spacing + swidth * 2),
                    ),
                    fill=switchcolor if up else None,
                )
            )  # Upper
            self.segments.append(
                SegmentPoly(
                    ...
                    fill=switchcolor if down else None,
                )
            )  # Lower
