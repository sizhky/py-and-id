    ...
    def __init__(
        ...
        **kwargs
        ...
        for row in range(rows):
            self.segments.append(
                Segment([(0, row * boxw), (w, row * boxw)], **kwargs)
            )
        for col in range(cols):
            self.segments.append(
                Segment([(col * boxw, 0), (col * boxw, h)], **kwargs)
            )
        ...
        for i, col in enumerate(order[:cols]):
            for j, row in enumerate(order[:rows]):
                invalue = "".join(
                    [k for k in format(i, colfmt)]
                    + [k for k in format(j, rowfmt)]
                )
                try:
        ...
        for group, style in groups.items():
            ...
            gwidth = int(
                2**tlen
                / (2 ** (group[:tlen].count("0") + group[:tlen].count("1")))
            )
            gheight = int(
                2**llen
                / (2 ** (group[tlen:].count("0") + group[tlen:].count("1")))
            ...
            for vert in verts:
                self.segments.append(
                    SegmentPoly(vert, cornerradius=0.25, **style)
                )
