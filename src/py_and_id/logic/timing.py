...
class TimingDiagram(Element):
    ...
    def __init__(self, waved: dict[str, str], **kwargs):
        ...
        self.yheight = kwargs.pop("yheight", 0.5)
        self.ygap = kwargs.pop("ygap", 0.3)
        self.risetime = kwargs.pop("risetime", 0.15)
        self.fontsize = kwargs.pop("fontsize", 12)
        self.nodesize = kwargs.pop("nodesize", 8)
        self.namecolor = kwargs.pop("namecolor", "blue")
        self.datacolor = kwargs.pop(
            "datacolor", None
        )  # default: get color from theme
        ...
        clipbox = BBox(
            0, self.yheight, periods * self.yheight * 2 * self.hscale, -height
        )
    ...
    def _drawwave(self, signal, y0=0):
        ...
        wavekwargs = ChainMap(
            {"color": signal.get("color", None), "lw": signal.get("lw", 1)},
            self.kwargs,
        )
    ...
    def _drawnodes(self, signal, y0):
        ...
        for j, node in enumerate(nodes):
            ...
            if (
                not node.isupper()
            ):  # Only uppercase nodes and symbols are drawn
                self.segments.append(
                    SegmentPoly(
                        [
    ...
    def _drawedges(self):
        ...
        for edge in edges:
            ...
            if mode == "-":  # Straight line
                ...
                p0 = Point(
                    (
                        p0.x + chrrad * math.cos(th0),
                        p0.y + chrrad * math.sin(th0),
                    )
                )
                pn = Point(
                    (
                        pn.x - chrrad * math.cos(th0),
                        pn.y - chrrad * math.sin(th0),
                    )
                )
                self.segments.append(
                    Segment(
                ...
                self.segments.append(
                    Segment((p0, pn), lw=1, ls=ls, color=color)
                )
                cap1 = [
                    (
                        p0.x + caplen * math.cos(th0),
                        p0.y + caplen * math.sin(th0),
                    ),
                    (
                        p0.x - caplen * math.cos(th0),
                        p0.y - caplen * math.sin(th0),
                ...
                cap2 = [
                    (
                        pn.x + caplen * math.cos(th0),
                        pn.y + caplen * math.sin(th0),
                    ),
                    (
                        pn.x - caplen * math.cos(th0),
                        pn.y - caplen * math.sin(th0),
                ...
                center = Point(
                    (p0.x + 0.8 * (pn.x - p0.x), p0.y + (pn.y - p0.y) / 2)
                )
                dx = pn.x - p0.x
                ...
                center = Point(
                    (p0.x + 0.25 * (pn.x - p0.x), (p0.y + pn.y) / 2)
                )
                dx = pn.x - p0.x
    ...
    def _drawgroups(self, signals, labelwidth):
        ...
        for label in getlabels(signals):
            ...
            ytop = (
                -label.row * (self.yheight + self.ygap)
                + self.yheight
                - edgelen
            )
            ybot = (
