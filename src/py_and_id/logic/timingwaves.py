...
class Doublesigmoid:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.gap = (
            gap  # Distance from first to second curve, relative to height
        )
        self.kwargs = kwargs
    ...
    def curve(
        ...
        sigy = [
            1 / (1 + math.exp(-x * self.rate)) * fullh - drop for x in sigx
        ]

        if crop:
            sigx = [
                x for i, x in enumerate(sigx) if 0 <= sigy[i] < self.height
            ]
            sigy = [
                y for i, y in enumerate(sigy) if 0 <= sigy[i] < self.height
            ]
...
class Wavez(Wave0):
    ...
    def verts_in(self) -> list[tuple[float, float]]:
        ...
        verts = {
            ...
            "V": [
                (self.xrise + self.rise, self.yhalf)
            ],  # V gets curves on its output
        }.get(self.plevel, [])
        return verts
...
class WaveV(Wave0):
    ...
    def verts_out(self) -> list[tuple[float, float]]:
        ...
        verts = {
            ...
            "1": [
                (self.xend, self.y0),
                (self.xend + self.rise, self.y1),
            ],  # Rise
            "H": [(self.xend, self.y0), (self.xend, self.y1)],
            "h": [(self.xend, self.y0), (self.xend, self.y1)],
            "z": list(
                zip(xcurve, [yc - (self.y1 - self.y0) / 2 for yc in ycurvehf])
            )
            + list(
                zip(
                    xcurve[::-1],
    ...
    def segments(self) -> list[SegmentType]:
        ...
        if (
            ...
            segments.append(
                Segment(
                    [(self.x0, self.y0), (self.xend, self.y0)], **self.kwargs
                )
            )
            segments.append(
                Segment(
                    [(self.x0, self.y1), (self.xend, self.y1)], **self.kwargs
                )
            )
            segments.append(
                SegmentPoly(
            ...
            segments.append(
                SegmentPoly(self.verts_in() + self.verts_out(), **ukwargs)
            )

        if (
            self.params.get("data", None)
            and self.params.get("state", None) != "x"
        ):
            segments.append(
                SegmentText(
...
class WaveClk(Wave0):
    ...
    def verts_in(self) -> list[tuple[float, float]]:
        ...
        if (
            self.params["state"] in "nN" and self.params["pstate"] in "lL"
        ) or (self.params["state"] in "pP" and self.params["pstate"] in "hH"):
