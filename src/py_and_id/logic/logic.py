        ...
        **kwargs
        ...
        for i in range(inputs):
            ...
            if inputnots and (inputs - i) in inputnots:
                self.segments.append(
                    SegmentCircle((leadin - notbubble, y), notbubble)
                )
                self.segments.append(
                    Segment([(0, y), (leadin - notbubble * 2, y)])
                )
            else:
        ...
        if nand:
            self.segments.append(
                Segment(
                    [
                        (gatel + gateh / 2 + leadin + notbubble * 2, 0),
                        (gatel + gateh / 2 + leadin + leadout, 0),
                    ]
                )
            )
        else:
...
class Or(Element):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if nor:
            self.segments.append(
                SegmentCircle((tip + notbubble, 0), notbubble)
            )
        ...
        for i in range(inputs):
            ...
            if inputnots and (inputs - i) in inputnots:
                self.segments.append(
                    SegmentCircle((xback - notbubble, y), notbubble)
                )
                self.segments.append(
                    Segment([(0, y), (xback - notbubble * 2, y)])
                )
            else:
        ...
        if inputs > 3:
            self.segments.append(
                Segment([(leadin, backlen / 2 + dy / 2), (leadin, orheight)])
            )
            self.segments.append(
                Segment([(leadin, -backlen / 2 - dy / 2), (leadin, -orheight)])
            )
        ...
        if nor:
            self.segments.append(
                Segment([(tip + notbubble * 2, 0), (tip + leadout, 0)])
            )
        else:
...
class Not(Element2Term):
    ...
    def __init__(self, *d, **kwargs):
        super().__init__(*d, **kwargs)
        self.segments.append(
            Segment([(0, 0), gap, (gatel + notbubble * 2, 0)])
        )
        self.segments.append(
            Segment(
                [(0, gateh / 2), (gatel, 0), (0, -gateh / 2), (0, gateh / 2)]
...
class Tgate(Element2Term):
    ...
    def __init__(self, *d, **kwargs):
        ...
        self.segments.append(
            Segment([(gatel / 2, 0.28 + 0.16), (gatel / 2, 0.7)])
        )
...
class SchmittAnd(And):
    ...
    def __init__(
        self, *d, leadin: float = 0.35, leadout: float = 0.35, **kwargs
    ):
