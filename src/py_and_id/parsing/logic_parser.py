...
def parse_string(logicstr):
    ...
    identifier = ~(
        and_ | or_ | nand_ | nor_ | not_ | true_ | false_
    ) + pyparsing.Word(
...
def drawlogic(tree, gateH=0.7, gateW=2, outlabel=None):
    ...
    def drawit(root, depth=0, outlabel=None):
        ...
        for i, child in enumerate(root.children):
            anchorname = (
                "start" if elm in [logic.Not, logic.Buf] else f"in{i+1}"
            )
...
def logicparse(
    expr: str, gateW: float = 2, gateH: float = 0.75, outlabel: str = None
) -> py_and_id.Drawing:
