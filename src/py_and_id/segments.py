...
def roundcorners(verts: Sequence[XY], radius: float = 0.5) -> Sequence[XY]:
    ...
    for v in range(len(verts))[::-1]:
        ...
        endangle = math.atan2(
            p2cross[1] - circlepoint[1], p2cross[0] - circlepoint[0]
        )
...
class Segment:
    ...
    def doreverse(self, centerx: float) -> None:
        ...
        if self.arrow:
            self.arrow = self.arrow[::-1].translate(
                self.arrow.maketrans("<>", "><")
            )
    ...
    def draw(self, fig, transform, **style) -> None:
        ...
        linepath = list(path)
        zorder = (
            self.zorder if self.zorder is not None else style.get("zorder", 2)
        )
        ...
        capstyle = (
            self.capstyle if self.capstyle else style.get("capstyle", "round")
        )
        joinstyle = (
            self.joinstyle
            if self.joinstyle
            else style.get("joinstyle", "round")
        )

        if fill:  # Check if path is closed
            tlist = list(map(tuple, path))  # Need path as tuples for set()
            dofill = len(tlist) != len(
                set(tlist)
            )  # Path has duplicates, can fill it
            if not dofill:
                fill = None
            elif fill is True:
                fill = color
        ...
        if self.arrow:
            ...
            if ">" in self.arrow:
                theta = math.degrees(
                    math.atan2(
                        path[-1].y - path[-2].y, path[-1].x - path[-2].x
                    )
                )
                fig.arrow(
                    path[-1],
...
class SegmentText:
    ...
    def xform(self, transform, **style) -> "SegmentText":
        ...
        params: dict[str, Any] = {
            ...
            "mathfont": self.mathfont
            if self.mathfont
            else style.get("mathfont", None),
    ...
    def get_bbox(self) -> BBox:
        ...
        return BBox(x - 0.2, y - 0.2, x + w + 0.4, y + h + 0.4)
    ...
    def draw(self, fig, transform, **style) -> None:
        ...
        mathfont = (
            self.mathfont if self.mathfont else style.get("mathfont", None)
        )
        align = (
            self.align
            if self.align
            else style.get("align", ("center", "center"))
        )
        ...
        zorder = (
            self.zorder if self.zorder is not None else style.get("zorder", 3)
        )
...
class SegmentPoly:
    ...
    def draw(self, fig, transform, **style) -> None:
        ...
        zorder = (
            self.zorder if self.zorder is not None else style.get("zorder", 1)
        )
        lw = self.lw if self.lw else style.get("lw", 2)
        ls = self.ls if self.ls else style.get("ls", "-")
...
class SegmentArc:
    ...
    def xform(self, transform, **style) -> "SegmentArc":
        ...
        return SegmentArc(
            transform.transform(self.center),
            self.width * transform.zoom,
            self.height * transform.zoom,
            angle=angle,
            theta1=self.theta1,
            theta2=self.theta2,
            **params,
    ...
    def get_bbox(self) -> BBox:
        ...
        t1 = math.atan2(
            self.width * math.sin(theta1), self.height * math.cos(theta1)
        )
        t2 = math.atan2(
            self.width * math.sin(theta2), self.height * math.cos(theta2)
        )
        while t2 < t1:
            t2 += 2 * math.pi
...
class SegmentArrow(Segment):
    ...
    def __init__(
        ...
        warnings.warn(
            "SegmentArrow is deprecated. Use Segment with arrow parameter.",
            DeprecationWarning,
        )
...
SegmentType = Union[
    Segment, SegmentText, SegmentPoly, SegmentArc, SegmentCircle, SegmentBezier
]
