...
def settextmode(mode: TextMode, svg2: bool = True) -> None:
    ...
    """
    warnings.warn(
        "settextmode is deprecated. Use py_and_id.svgconfig",
        DeprecationWarning,
    )
    config.svg2 = svg2
    config.text = mode
...
def text_size(
    ...
    if ziamath:
        ...
        for line in lines:
            maths.append(
                ziamath.Math.fromlatextext(
                    line, size=size, mathstyle=font, textstyle=font
                )
            )
...
class Figure:
    ...
    def __init__(self, bbox: BBox, **kwargs):
        self.svgelements: list[
            tuple[int, ET.Element]
        ] = []  # (zorder, element)
        self.hatch: bool = False
        self.clips: dict[BBox, str] = {}
        self.showframe = kwargs.get("showframe", False)
        self.scale = 64.8 * kwargs.get(
            "inches_per_unit", 0.5
        )  # Magic scale factor that matches what MPL did
        self.set_bbox(bbox)
    ...
    def arrow(
        ...
        fin1 = (
            Point((fullen - arrowlength, arrowwidth / 2)).rotate(theta) + tail
        )
        fin2 = (
            Point((fullen - arrowlength, -arrowwidth / 2)).rotate(theta) + tail
        )
    ...
    def arc(
        ...
        if arrow is not None:
            ...
            x, y = math.cos(math.radians(theta2)), math.sin(
                math.radians(theta2)
            )
            th2 = math.degrees(math.atan2((width / height) * y, x))
            x, y = math.cos(math.radians(theta1)), math.sin(
                math.radians(theta1)
            )
    ...
    def getimage(self, ext: str = "svg") -> bytes:
        ...
        svg.set(
            "viewBox", f"{x0} {y0} {self.pxwidth+2*pad} {self.pxheight+2*pad}"
        )
        if self._bgcolor:
