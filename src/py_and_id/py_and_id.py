...
class Drawing:
    ...
    def get_segments(self) -> list[SegmentType]:
        ...
        for element in self.elements:
            ...
            segments.extend(
                [
                    s.xform(element.transform, **params)
    ...
    def add(self, element: Element | Type[Element], **kwargs) -> Element:
        ...
        self.here, self.theta = element._place(
            self.here, self.theta, **self.dwgparams
        )
    ...
    def move_from(
        self, ref: Point, dx: float = 0, dy: float = 0, theta: float = None
    ) -> None:
    ...
    def _initfig(
        self, ax=None, backend: Backends = None, showframe: bool = False
    ) -> None:
        figclass = {"matplotlib": mplFigure, "svg": svgFigure}.get(
            backend, Figure
        )  # type: ignore
        fig = figclass(
            ax=ax,
    ...
    def draw(
        ...
        if (
            self.fig is None
            or ax is not None
            or showframe != self.fig.showframe
    ...
    def save(
        self, fname: str, transparent: bool = True, dpi: float = 72
    ) -> None:
