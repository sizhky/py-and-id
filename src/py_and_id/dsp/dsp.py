...
class SumSigma(Circle):
    ...
    def __init__(self, *d, **kwargs):
        ...
        self.segments.append(
            SegmentText(
                label=r"$\Sigma$", pos=(0.45, 0), align=("center", "center")
            )
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        if N:
            self.segments.append(
                SegmentText(
                    (rad, rad / 2 + 0.01), N, font=font, fontsize=fontsize
                )
            )
        if S:
            self.segments.append(
                SegmentText(
                    (rad, -rad / 2 - 0.065), S, font=font, fontsize=fontsize
                )
            )
        if E:
            self.segments.append(
                SegmentText(
                    (rad * 1.5 + 0.05, 0), E, font=font, fontsize=fontsize
                )
            )
        if W:
            self.segments.append(
                SegmentText(
                    (rad / 2 - 0.05, 0), W, font=font, fontsize=fontsize
                )
            )
...
class Filter(Square):
    ...
    def __init__(self, *d, response: FilterType = None, **kwargs):
        super().__init__(*d, **kwargs)
        path = _makesine()
        path1 = [[p[0], p[1] + 0.25] for p in path]
        path2 = [[p[0], p[1] - 0.25] for p in path]
        self.segments.append(Segment(path))
