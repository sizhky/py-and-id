...
class Box(Element):
    ...
    def _place(self, dwgxy, dwgtheta, **dwgparams):
        ...
        if "anchor" not in self._userparams and "drop" not in self.params:
            ...
            idx = min(
                range(len(thetas)), key=lambda i: abs(thetas[i] - dwgtheta)
            )
            anchor = anchors[idx]
...
class RoundBox(Box):
    ...
    def __init__(
        self, w: float = 3, h: float = 2, cornerradius: float = 0.3, **kwargs
    ):
...
class StateEnd(Connect):
    ...
    def __init__(self, r: float = 0.75, dr=0.15, **kwargs):
